                ...
                <table class="table table-bordered table-hover table-striped table-light yajra-datatable">
                    <thead class="table-dark ">
                        <tr class="text-light">
                            ...
                            <th class="text-light">عدد المحافظات</th>
                    ...
                    <tbody>
                        @foreach ($countries as $k => $country)
                            <tr data-entry-id="{{ $country->id }}">
                                ...
                                <td class="text-center">{{ $country->states->count() }}</td>
                                <td class="text-center">
                                    <span class="switch switch-icon">
                                        <label>
                                            <input data-id="{{ $country->id }}" class="status-class" type="checkbox"
                                ...
                                <td class="text-center">
                                            ...
                                            <a href="javascript:void(0)"
                                                onclick="
                                                    if (confirm('Are You Sure You Want To Delete This Record ?') )
                                                        { document.getElementById('record_delete_{{ $country->id }}').submit(); }
                                                    else
                                                        { return false; }"
                                    ...
                                    <form action="{{ route('admin.countries.destroy', $country->id) }}" method="post" id="record_delete_{{ $country->id }}" class="d-none">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </td>
        ...
        $(function () {
            $('.status-class').change(function() {
                ...
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    url: '{{ route('admin.countries.changeStatus') }}',
                    data: {
                        'status': status,
                        'cat_id': cat_id
                    },
