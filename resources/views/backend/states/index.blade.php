                    ...
                    <tbody>
                        @foreach ($states as $k => $state)
                            <tr data-entry-id="{{ $state->id }}">
                                ...
                                <td class="text-center">{{ $state->cities->count() }}</td>
                                <td class="text-center">
                                    <span class="switch switch-icon">
                                        <label>
                                            <input data-id="{{ $state->id }}" class="status-class" type="checkbox"
                                ...
                                <td class="text-center">
                                            ...
                                            <a href="javascript:void(0)"
                                                onclick="
                                                    if (confirm('Are You Sure You Want To Delete This Record ?') )
                                                        { document.getElementById('record_delete_{{ $state->id }}').submit(); }
                                                    else
                                                        { return false; }"
                                    ...
                                    <form action="{{ route('admin.states.destroy', $state->id) }}" method="post" id="record_delete_{{ $state->id }}" class="d-none">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </td>
        ...
        $(function () {
            $('.status-class').change(function() {
                ...
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    url: '{{ route('admin.states.changeStatus') }}',
                    data: {
                        'status': status,
                        'cat_id': cat_id
                    },
